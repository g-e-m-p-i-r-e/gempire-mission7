...
function AuthNotConfigured() {
  return <div className="text-yellow-400 text-sm">Authentication not configured</div>;
}
...
function PrivyAuth({ onAddressChange }: { onAddressChange: (address: string) => void }) {
  ...
  const { user: monadUser, hasUsername, isLoading: isLoadingUser, error: userError } = useMonadGamesUser(accountAddress);
