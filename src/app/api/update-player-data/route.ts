...
import { createAuthenticatedResponse, validateOrigin, validateSessionToken } from '@/app/lib/auth';
import { rateLimit } from '@/app/lib/rate-limiter';
import { CONTRACT_ABI, CONTRACT_ADDRESS, isValidAddress } from '@/app/lib/blockchain';
import { generateRequestId, isDuplicateRequest, markRequestComplete, markRequestProcessing } from '@/app/lib/request-deduplication';
import { GAME_CONFIG } from '@/app/lib/game-config';
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!playerAddress || scoreAmount === undefined || transactionAmount === undefined) {
      return createAuthenticatedResponse({ error: 'Missing required fields: playerAddress, scoreAmount, transactionAmount' }, 400);
    }

    // Validate player address format
    if (!isValidAddress(playerAddress)) {
      return createAuthenticatedResponse({ error: 'Invalid player address format' }, 400);
    }

    // Validate that scoreAmount and transactionAmount are positive numbers
    if (scoreAmount < 0 || transactionAmount < 0) {
      return createAuthenticatedResponse({ error: 'Score and transaction amounts must be non-negative' }, 400);
    }



    // Maximum limits to prevent abuse - made more restrictive
    const MAX_SCORE_PER_REQUEST = GAME_CONFIG.SCORE_SUBMISSION.SCORE_THRESHOLD; // Reduced from 10000
    const MAX_TRANSACTIONS_PER_REQUEST = GAME_CONFIG.SCORE_SUBMISSION.TRANSACTION_THRESHOLD; // Reduced from 100

    // Additional validation: reasonable score ranges
    const MIN_SCORE_PER_REQUEST = 1;
    const MAX_SCORE_PER_TRANSACTION = 100000; // Max 100 points per transaction
    ...
    if (scoreAmount < MIN_SCORE_PER_REQUEST && scoreAmount !== 0) {
      return createAuthenticatedResponse({ error: `Score amount too small. Minimum: ${MIN_SCORE_PER_REQUEST}` }, 400);
    }
    ...
    if (isDuplicateRequest(requestId)) {
      return createAuthenticatedResponse({ error: 'Duplicate request detected. Please wait before retrying.' }, 409);
    }
    ...
    const hash = await walletClient.writeContract({
      ...
      args: [playerAddress as `0x${string}`, BigInt(scoreAmount), BigInt(transactionAmount)],
    });
    ...
    if (error instanceof Error) {
      if (error.message.includes('insufficient funds')) {
        return createAuthenticatedResponse({ error: 'Insufficient funds to complete transaction' }, 400);
      }
      if (error.message.includes('execution reverted')) {
        return createAuthenticatedResponse({ error: 'Contract execution failed - check if wallet has GAME_ROLE permission' }, 400);
      }
      if (error.message.includes('AccessControlUnauthorizedAccount')) {
        return createAuthenticatedResponse({ error: 'Unauthorized: Wallet does not have GAME_ROLE permission' }, 403);
      }
    }

    return createAuthenticatedResponse({ error: 'Failed to update player data' }, 500);
  }
}
