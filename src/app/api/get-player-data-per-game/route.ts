...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!playerAddress || !gameAddress) {
      return NextResponse.json({ error: 'Both playerAddress and gameAddress are required' }, { status: 400 });
    }

    if (!isValidAddress(playerAddress) || !isValidAddress(gameAddress)) {
      return NextResponse.json({ error: 'Invalid player or game address format' }, { status: 400 });
    }
    ...
    return NextResponse.json({ error: 'Failed to get player data per game' }, { status: 500 });
  }
}
