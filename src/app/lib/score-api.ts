...
export async function getSessionToken(playerAddress: string): Promise<string | undefined> {
  try {
    const message = `Authenticate for score submission: ${playerAddress}`;
    const signedMessage = 'dummy_signature';

    const response = await fetch('/api/get-session-token', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ playerAddress, message, signedMessage }),
    });
...
export async function getPlayerGameData(playerAddress: string, gameAddress: string): Promise<PlayerDataPerGameResponse | null> {
  try {
  ...
  private async processQueue() {
    ...
    for (const transaction of readyTransactions) {
      try {
        ...
        const result = await submitPlayerScore(transaction.playerAddress, transaction.scoreAmount, transaction.transactionAmount);
        ...
        if (result.success) {
          ...
          const isPriorityError =
            result.error?.includes('Another transaction has higher priority') || result.error?.includes('higher priority');
          ...
          if (transaction.attempts >= transaction.maxAttempts) {
            ...
            if (isPriorityError) {
              ...
              backoffDelay = Math.min(3000 + Math.random() * 5000 + 1000 * transaction.attempts, 60000); // 3-8s + attempt penalty, cap at 60s
            } else {
        ...
        const isPriorityError =
          errorMessage.includes('Another transaction has higher priority') || errorMessage.includes('higher priority');
